"""
Django app configuration for Auth Service with Consul integration
"""
import sys
import os
import atexit
...
class AuthServiceConfig(AppConfig):
    ...
    def ready(self):
        ...
        """
        print("[DEBUG] AuthServiceConfig.ready() called")
        
        # Only register in main process (not in reloader or migration processes)
        # RUN_MAIN is set by Django's autoreloader
        ...
        if not use_consul:
            ...
            logger.info("Consul integration disabled (USE_CONSUL=False)")
            return
        ...
        try:
            ...
            # Identify which agent we are talking to
            try:
                agent_self = consul_client.consul.agent.self()
                node_name = agent_self.get('Config', {}).get('NodeName', 'unknown')
                print(f"[DEBUG] Connected to Consul Agent Node: {node_name}")
            except:
                print("[DEBUG] Connected to Consul but could not get agent info")
            
            print("[DEBUG] Consul client is available!")
            
            # Service configuration
            service_name = 'auth-service'
            service_id = f"auth-service-{os.getpid()}"
            service_port = 8001
            ...
            if success:
                print(f"✅ Auth Service registered with Consul (ID: {service_id})")
                logger.info(f"✓ Auth Service registered with Consul (ID: {service_id})")
            ...
            # Don't crash the application if Consul registration fails