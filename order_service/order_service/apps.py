"""
Django app configuration for Order Service with Consul integration
"""
import sys
import os
import atexit
...
class OrderServiceConfig(AppConfig):
    ...
    def ready(self):
        ...
        print("[DEBUG] OrderServiceConfig.ready() called")
        ...
        try:
            ...
            service_id = f"order-service-{os.getpid()}"
            service_port = 8003
            ...
            if success:
                print(f"✅ Order Service registered with Consul (ID: {service_id})")
                logger.info(f"✓ Order Service registered with Consul (ID: {service_id})")
