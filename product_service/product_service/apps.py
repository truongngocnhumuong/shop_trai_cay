"""
Django app configuration for Product Service with Consul integration
"""
import sys
import os
import atexit
...
class ProductServiceConfig(AppConfig):
    """Product Service application configuration"""
    default_auto_field = 'django.db.models.BigAutoField'
    name = 'product_service'
    
    def ready(self):
        """Register service with Consul if enabled"""
        print("[DEBUG] ProductServiceConfig.ready() called")
        ...
        try:
            ...
            service_id = f"product-service-{os.getpid()}"
            service_port = 8002
            ...
            if success:
                print(f"✅ Product Service registered with Consul (ID: {service_id})")
                logger.info(f"✓ Product Service registered with Consul (ID: {service_id})")
